...
async def test_admin_creation_permissions():
    async with AsyncClient(base_url="http://127.0.0.1:8000") as ac:
        ...
        response = await ac.post("/api/v1/auth/login", json={"username": "admin", "password": "admin"})
        assert response.status_code == 200
        main_token = response.json()["access_token"]
        
        # 2. Main admin creates sub-admin
        res = await ac.post("/api/v1/admin/users", json={"role": "admin"}, headers={"Authorization": f"Bearer {main_token}"})
        ...
        res = await ac.post("/api/v1/auth/login", json={"username": sub_admin, "password": sub_admin_pw})
        assert res.status_code == 200
        sub_admin_token = res.json()["access_token"]
        
        # 5. Sub admin cannot create admin
        res = await ac.post("/api/v1/admin/users", json={"role": "admin"}, headers={"Authorization": f"Bearer {sub_admin_token}"})
        assert res.status_code == 403
        
        # 6. Sub admin can create mod
        res = await ac.post("/api/v1/admin/users", json={"role": "moderator"}, headers={"Authorization": f"Bearer {sub_admin_token}"})
        assert res.status_code == 201
