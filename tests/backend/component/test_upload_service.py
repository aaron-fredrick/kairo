...
from src.backend.services.upload_service import confirm_upload
from src.backend.storage.paths import temp_upload_path
from src.backend.storage.provider import LocalStorageProvider
...
def storage_provider(tmp_path, monkeypatch):
    ...
    from src.backend.storage.blob_manager import BlobManager

    prov = LocalStorageProvider(str(tmp_path))
    mgr = BlobManager(prov)
    provider_mod = importlib.import_module("src.backend.storage.provider")
    blob_mod = importlib.import_module("src.backend.storage.blob_manager")
    upload_mod = importlib.import_module("src.backend.services.upload_service")
