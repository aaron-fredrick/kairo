...
The **app-register** service (`src/register/`) tracks healthy **app-backend** nodes and publishes reverse-proxy configuration (Caddy or Nginx) so a single edge proxy can load-balance HTTP and WebSocket traffic.

## Layered design

```
┌─────────────────────────────────────────────────────────────┐
│  HTTP API (src/register/api.py)                                 │
│  HMAC auth, request/response models                         │
└───────────────────────────┬─────────────────────────────────┘
                            │
┌───────────────────────────▼─────────────────────────────────┐
│  Application (src/register/services/coordinator.py)             │
...
│  (src/register/ports/registry)  │   │  ProxyConfigPublisher       │
└───────────────┬──────────────┘   └──────────┬─────────────────┘
                │                             │
┌───────────────▼──────────────┐   ┌──────────▼─────────────────┐
│  Adapters                     │   │  Adapters                   │
...
| **Container** | `src/register/container.py` wires default adapters from `RegisterSettings`. |
...
1. Background task in `src/register/main.py` runs every `HEARTBEAT_INTERVAL_SECONDS`.
...
- **`src/backend/clients/register_client.py`**: optional client that registers on app-backend lifespan and runs the heartbeat loop.
